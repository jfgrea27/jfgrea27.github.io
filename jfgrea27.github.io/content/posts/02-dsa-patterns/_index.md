...
The _Worst case $\Omicron(n)$_ describes the maximum time/space an algorithm takes for _any_ input. It is the **guaranteed upper bound**.

The _Average case $\Omicron(n)$_ describes the expected time across all posbbile inputs. It tells you what you **expect typically**.
...
As discussed in the [recap on complexity](#a-quick-recap-on-algorithmic-complexity) above, appending at end will have an average-case of $\Omicron(1)$ and a worst-case $\Omicron(n)$. We call this _ammortized_ $\Omicron(1)$ (= when certain infrequent operations make other operations fast).
